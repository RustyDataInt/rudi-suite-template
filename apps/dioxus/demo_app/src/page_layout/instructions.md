...
Thus, you can have one panel with a width of 12 that 
...
the panels will cascade in a vertical stack.

### Building an app step component

Usually, the only place you need to implement the fluid page layout
is in an `AppStepPage` following this pattern:

```rust
// .../dioxus/<app_name>/src/<app_step_name>/mod.rs
#[component]
pub fn MyAppStep() -> Element {
    let this = RudiElement::app_step::<()>("app_step_name");
    rsx! {
